...
def selects_most_prominent_color_and_returns_with_count
  "SELECT MAX(color), COUNT(color) FROM bears GROUP BY color HAVING COUNT(color) = (SELECT MAX(color) FROM ( SELECT agent_code, COUNT(agent_code) mycount FROM orders GROUP BY agent_code)); ;"
end
